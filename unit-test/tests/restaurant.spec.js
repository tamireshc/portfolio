...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifique se o retorno da função createMenu() é um objeto que possui a chave fetchMenu, a qual tem como valor uma função', () => {
    ...
    Object.keys(createMenu());
    Object.values(createMenu());
  ...
  it('Verifique se se o menu passado pra função createMenu é identico ao menu recuperado pela função "objetoRetornado.fetchMenu"', () => {
    const objetoRetornado = createMenu({ objetoQualquer: 'objeto' });
    expect(objetoRetornado.fetchMenu()).toEqual({ objetoQualquer: 'objeto' });
  });
  ...
  it('Verifique ao chamar uma função associada à chave `order` no objeto retornado,passando uma string como parâmetro (como `objetoRetornado.order("coxinha")`), tal string é adicionada ao array retornado em `objetoRetornado.consumption`.', () => {
    const objetoRetornado = createMenu({
      food: { coxinha: 3.9, sanduiche: 9.9 },
      drinks: { agua: 3.9, cerveja: 6.9 },
    });
  ...
  it('Verifique ao adicionar três pedidos, dentre bebidas e comidas, o array `objetoRetornado.consumption` contém os itens pedidos..', () => {
    const objetoRetornado = createMenu({
      food: { coxinha: 3.9, sanduiche: 9.9 },
      drinks: { agua: 3.9, cerveja: 6.9 },
    });
  ...
  it('Verifique se ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`', () => {
    ...
    expect(objetoRetornado.pay()).toBe(27);
  });
});
