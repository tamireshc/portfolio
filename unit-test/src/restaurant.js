...
const consumedValue = (myMenu) => {
  const values = Object.values(myMenu);
  const valuesFood = Object.values(values[0]);
  const valuesDrink = Object.values(values[1]);

  const totalFood = valuesFood.reduce((a, b) => a + b);
  const totalDrink = valuesDrink.reduce((a, b) => a + b);
  const total = Math.round((totalFood + totalDrink) * 1.1);
  return total;
};
...
const createMenu = (myMenu) => {
  const menu = () => myMenu;
  const pay = () => consumedValue(myMenu);
